...
PLUG_VERSION=1.0.5.6   # 插件版本
IS_PROD=0              # 1 表示 true，即生产环境；0 表示 false，即测试环境

COMPONENT=utils-titans-ficc
...
source ~/.bashrc
cd /usr/local/gopath/src/rhino-applications/titans-ficc/utils
...
# 编译 x86 版本
echo "开始编译 x86 版本..."
GO111MODULE=off CGO_ENABLED=1 GOOS=linux GOARCH=amd64 /usr/local/go/bin/go build -o artifacts/x86/utils cmd/*.go
echo "x86 版本编译完成！"

# 编译 arm64 版本
echo "开始编译 arm64 版本..."
GO111MODULE=off CGO_ENABLED=1 GOOS=linux GOARCH=arm64 CC=aarch64-linux-gnu-gcc /usr/local/go/bin/go build -o artifacts/arm/utils cmd/*.go
echo "arm64 版本编译完成！"
EOF
...
# 配置日志轮转规则（每日00:00触发）
RUN echo "/opt/log/utils.log {"  > /etc/logrotate.d/utils && \\
    echo "  daily"                     >> /etc/logrotate.d/utils && \\
    echo "  rotate 7"                  >> /etc/logrotate.d/utils && \\
    echo "  compress"                  >> /etc/logrotate.d/utils && \\
    echo "  delaycompress"             >> /etc/logrotate.d/utils && \\
    echo "  missingok"                 >> /etc/logrotate.d/utils && \\
    echo "  notifempty"                >> /etc/logrotate.d/utils && \\
    echo "  dateext"                   >> /etc/logrotate.d/utils && \\
    echo "  copytruncate"              >> /etc/logrotate.d/utils && \\
    echo "  create 644 root root"      >> /etc/logrotate.d/utils && \\
    echo "}"

# 设置定时任务（每日00:00执行）
RUN echo "0 0 * * * root /usr/sbin/logrotate -f /etc/logrotate.d/utils" > /etc/cron.d/trade-logrotate && \\
    chmod 644 /etc/cron.d/trade-logrotate


WORKDIR /opt/log
VOLUME /opt/log
CMD ["/bin/sh", "-c", "service cron start;/$COMPONENT 2>&1 | tee -a /opt/log/utils.log"]
EOF

scp eagle@10.51.136.23:/usr/local/gopath/src/rhino-applications/titans-ficc/utils/artifacts/x86/utils ./docker/$COMPONENT
...
scp eagle@10.51.136.23:/usr/local/gopath/src/rhino-applications/titans-ficc/utils/artifacts/arm/utils ./docker/$COMPONENT
