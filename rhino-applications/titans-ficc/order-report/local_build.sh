...
PLUG_VERSION=1.1.9.1   # 插件版本
IS_PROD=0              # 1 表示 true，即生产环境；0 表示 false，即测试环境

COMPONENT=order-report-titans-ficc
...
source ~/.bashrc
cd /usr/local/gopath/src/rhino-applications/titans-ficc/order-report
...
WORKDIR /opt/log
VOLUME /opt/log
CMD ["/bin/sh", "-c", "service cron start;/$COMPONENT 2>&1 | tee -a /opt/log/order-report.log"]
EOF

scp eagle@10.51.136.23:/usr/local/gopath/src/rhino-applications/titans-ficc/order-report/artifacts/x86/order-report ./docker/$COMPONENT
...
scp eagle@10.51.136.23:/usr/local/gopath/src/rhino-applications/titans-ficc/order-report/artifacts/arm/order-report ./docker/$COMPONENT
