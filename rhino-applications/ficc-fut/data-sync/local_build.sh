...
PLUG_VERSION=1.0.1.8   # 插件版本
IS_PROD=0              # 1 表示 true，即生产环境；0 表示 false，即测试环境

COMPONENT=data-sync-ficc-fut
...
source ~/.bashrc
cd /usr/local/gopath/src/rhino-applications/ficc-fut/data-sync
...
# 配置日志轮转规则（每日00:00触发）
RUN echo "/opt/log/data-sync.log {"  > /etc/logrotate.d/data-sync && \\
    echo "  daily"                     >> /etc/logrotate.d/data-sync && \\
    echo "  rotate 7"                  >> /etc/logrotate.d/data-sync && \\
    echo "  compress"                  >> /etc/logrotate.d/data-sync && \\
    echo "  delaycompress"             >> /etc/logrotate.d/data-sync && \\
    echo "  missingok"                 >> /etc/logrotate.d/data-sync && \\
    echo "  notifempty"                >> /etc/logrotate.d/data-sync && \\
    echo "  dateext"                   >> /etc/logrotate.d/data-sync && \\
    echo "  copytruncate"              >> /etc/logrotate.d/data-sync && \\
    echo "  create 644 root root"      >> /etc/logrotate.d/data-sync && \\
    echo "}"

# 设置定时任务（每日00:00执行）
RUN echo "0 0 * * * root /usr/sbin/logrotate -f /etc/logrotate.d/data-sync" > /etc/cron.d/trade-logrotate && \\
    chmod 644 /etc/cron.d/trade-logrotate


WORKDIR /opt/log
VOLUME /opt/log
CMD ["/bin/sh", "-c", "service cron start;/$COMPONENT 2>&1 | tee -a /opt/log/data-sync.log"]
EOF

scp eagle@10.51.136.23:/usr/local/gopath/src/rhino-applications/ficc-fut/data-sync/artifacts/x86/data-sync ./docker/$COMPONENT
...
scp eagle@10.51.136.23:/usr/local/gopath/src/rhino-applications/ficc-fut/data-sync/artifacts/arm/data-sync ./docker/$COMPONENT
