...
func (c *OltsFutChannel) OrderCancelRequest(actionUser string, actionTime int64, actionKey string, targetClOrdID string, streamInputMsgSeq int64, order *schema.TradeOrder, afterTradeActionLatestRespInsert func(tradeActionLatestResp *schema.TradeActionLatestResp), syncTradeActionLatestRespAfterError func(tradeActionLatestResp *schema.TradeActionLatestResp)) (de *domain_error.Error) {
	...
	// 发送FIX撤单指令
	de = c.doOrderCancelRequestByOlts(cancelAction, order)
...
// 实现FIX撤单指令
func (c *OltsFutChannel) doOrderCancelRequestByOlts(cancelAction *schema.TradeActionLatestResp, order *schema.TradeOrder) (de *domain_error.Error) {
	...
	cancelRequest := &OrderCancelRequest{
		...
		TransactTime: timeutil.ConvertMillisecondsToTime(cancelAction.TransactTime).In(timeutil.CnTimeLocation).Format(TransactTimeLayout),
		AlgoName:     order.AlgName,
	}

	if order.AlgName == "" {
		cancelRequest.AlgoName = "0"
	}
