...
type TitansFiccOrderStatusAdapter struct {
	strictCtpyMap  map[string]bool
	applicationCfg *domain_cfg.ApplicationCfg
	ytmutil        *ytmutil.YtmUtil
}

func NewTitansFiccOrderStatusAdapter(c *domain_cfg.ApplicationCfg) (adapter *TitansFiccOrderStatusAdapter, de *domain_error.Error) {
	log.Printf("construct TitansFiccOrderStatusAdapter...")

	adapter = &TitansFiccOrderStatusAdapter{applicationCfg: c, ytmutil: ytmutil.NewYtmUtil(c.GetApplicationCfgItemMap())}
	adapter.initStrictCtpyMap()
	return
}

func (a *TitansFiccOrderStatusAdapter) LookUpTraceableTradeActionResp(directOrderMap map[string]*types.TraceableTradeOrder, tradeActionResp *schema.TradeActionResp, tradeActionRespMap map[string]*types.TraceableTradeActionResp) (traceableTradeActionResp *types.TraceableTradeActionResp, ok bool) {
	...
	if (!fillExecType[tradeActionResp.ExecType] && tradeActionResp.OrdStatus == string(enum.OrdStatus_PendingCancel)) || tradeActionResp.OrdStatus == string(enum.OrdStatus_Canceled) || len(tradeActionResp.CxlRejResponseTo) > 0 {
		...
		if traceableTradeOrder == nil {
			// log.Printf("key for directMap, clOrdID:%s\n", clOrdID)
			// for k := range directOrderMap {
			// 	log.Printf("===> key=%s\n", k)
			// }
			return
		}
...
func (a *TitansFiccOrderStatusAdapter) UpdateTradeActionLatestResp(tradeActionResp *schema.TradeActionResp, traceableTradeActionResp *types.TraceableTradeActionResp) {
	...
	if fillExecType[tradeActionResp.ExecType] {
		...
		var parValue int64 = 100
		extendAttrMap := traceableTradeActionResp.GetTradeOrder().ExtendAttrMap
		val, ok, _ := attrutil.GetAttrValue(extendAttrMap, "parValue", enum.AttrValueType_INT)
		if ok {
			parValue = int64(val.(int))
		}

		// 转化为张
		//units := tradeActionResp.LastShares/int64(parValue)

		// if traceableTradeActionResp.GetTradeActionLatestResp().CumQty <= 0 {
		// 	traceableTradeActionResp.GetTradeActionLatestResp().CumQty = tradeActionResp.LastShares
		// 	traceableTradeActionResp.GetTradeActionLatestResp().LeavesQty = int64(traceableTradeActionResp.GetTradeOrder().OrderQty) - tradeActionResp.LastShares
		// 	log.Printf("initial set,  traceableTradeActionResp.GetTradeActionLatestResp().CumQty=%v, traceableTradeActionResp.GetTradeActionLatestResp().LeavesQty=%v\n", traceableTradeActionResp.GetTradeActionLatestResp().CumQty, traceableTradeActionResp.GetTradeActionLatestResp().LeavesQty)
		// }
		...
		totalAmount := (order.AvgPx*float64(order.CumQty/parValue) + float64(tradeActionResp.LastShares/parValue)*tradeActionResp.LastPx)
		tradeActionLatestResp.AvgPx = totalAmount / float64(tradeActionLatestResp.CumQty/parValue)
...
func (a *TitansFiccOrderStatusAdapter) UpdateOrderStatusForOrderCancelReject(tradeActionResp *schema.TradeActionResp, tradeActionLatestResp *schema.TradeActionLatestResp, order *schema.TradeOrder) {
...
func (a *TitansFiccOrderStatusAdapter) resetRespCommission(tradeActionResp *schema.TradeActionResp, tradeOrder *schema.TradeOrder) {
...
func (a *TitansFiccOrderStatusAdapter) UpdateOrderStatusForExecutionReport(tradeActionResp *schema.TradeActionResp, tradeActionLatestResp *schema.TradeActionLatestResp, tradeActionRespList []*schema.TradeActionResp, order *schema.TradeOrder, traceableTradeOrder *types.TraceableTradeOrder) (orderUpdateAttributes map[string]interface{}) {
	...
	if fillExecType[tradeActionResp.ExecType] {
		...
		if tradeActionResp.ExtendAttrMap != nil {
			...
			a.setRespPriceInfo(tradeActionResp, tradeActionLatestResp, tradeActionRespList, order, traceableTradeOrder)
			
			if _, ok := order.ExtendAttrMap["commissionRate1"]; !ok {
				tradeActionResp.ExtendAttrMap["respCommissionRate"] = order.ExtendAttrMap["commissionRate"]
				//tradeActionResp.ExtendAttrMap["respDirtyPriceWithFee"] = order.ExtendAttrMap["dirtyPriceWithFee"]
				a.resetRespCommission(tradeActionResp, order)
			} else {
				respHandlInst, _, _ := attrutil.GetAttrValue(tradeActionResp.ExtendAttrMap, "respHandlInst", enum.AttrValueType_STRING)

				log.Printf("respHandlInst:%v,  ExecID:%s, AppOrdID:%s\n", respHandlInst, tradeActionResp.ExecID, order.AppOrdID)

				switch respHandlInst.(string) {
				case "1":
					tradeActionResp.ExtendAttrMap["respCommissionRate"] = order.ExtendAttrMap["commissionRate1"]
					a.resetRespCommission(tradeActionResp, order)
				case "2", "3", "4":
					tradeActionResp.ExtendAttrMap["respCommissionRate"] = order.ExtendAttrMap["commissionRate3"]
					a.resetRespCommission(tradeActionResp, order)
				default:
					tradeActionResp.ExtendAttrMap["respCommissionRate"] = order.ExtendAttrMap["commissionRate"]
					//tradeActionResp.ExtendAttrMap["respDirtyPriceWithFee"] = order.ExtendAttrMap["dirtyPriceWithFee"]
	...
	a.setOrderPriceInfo(tradeActionResp, tradeActionLatestResp, tradeActionRespList, order, traceableTradeOrder, orderUpdateAttributes)
	
	/*_, ok, _ := attrutil.GetAttrValue(order.ExtendAttrMap, "commissionRate1", enum.AttrValueType_FLOAT)
	if EndStatus[tradeActionResp.OrdStatus] && ok {

		cumQty := float64(order.CumQty)
		if cumQty == 0 {
			return
		}
		...
		log.Printf("1.reconfig order ExtendAttrMap for order %s when order is ended! commissionRate:%v\n", order.ClOrdID, commissionRate)

		dirtyPrice, _, _ := attrutil.GetAttrValue(order.ExtendAttrMap, "dirtyPrice", enum.AttrValueType_FLOAT)
		if order.Side == "1" { // 买单
			order.ExtendAttrMap["commissionRate"] = commissionRate
			order.ExtendAttrMap["dirtyPriceWithFee"] = dirtyPrice.(float64) * (1 + commissionRate)
		} else { // 卖单
			order.ExtendAttrMap["commissionRate"] = commissionRate
			order.ExtendAttrMap["dirtyPriceWithFee"] = dirtyPrice.(float64) * (1 - commissionRate)
		}

		log.Printf("2.reconfig order ExtendAttrMap for order %s when order is ended! commissionRate:%v\n", order.ClOrdID, order.ExtendAttrMap["commissionRate"])

		orderUpdateAttributes["commissionRate"] = order.ExtendAttrMap["commissionRate"]
		orderUpdateAttributes["dirtyPriceWithFee"] = order.ExtendAttrMap["dirtyPriceWithFee"]
	}*/
