...
type TenaciousProducerWithBuffer struct {
	...
	bytesBufferKafka   *bytes.Buffer
	messageLogFile     *os.File
	tail               *tail.Tail
}

func NewTenaciousProducerWithBuffer(messageLogFilePath string, resetMessageLogOnStart bool, writeBufferBytes int, sendingTopic string, brokers []string, inputBuf int) (*TenaciousProducerWithBuffer, error) {
	inst := &TenaciousProducerWithBuffer{
		...
		bytesBufferKafka:   bytes.NewBuffer(nil),
...
func (k *TenaciousProducerWithBuffer) resetProducer() error {
/*
	// 配置 Kafka 生产者
...
func (k *TenaciousProducerWithBuffer) SendMessage(data []byte) {
	k.input <- data
}

func (k *TenaciousProducerWithBuffer) reset() {
	//k.flushSignal <- true
	time.Sleep(5 * time.Second)
	k.flush()
}

func (k *TenaciousProducerWithBuffer) doSendMessage() {
	go func() {
...
func (k *TenaciousProducerWithBuffer) kafkaMessageSend(data string) {
	if len(data) == 0 {
		return
	}
	k.bytesBufferKafka.WriteString(data)
	if data[len(data)-1] != '\n' {
		k.bytesBufferKafka.WriteByte('\n')
	}

	if k.bytesBufferKafka.Len() < k.writeBufferBytes {
	//if k.bytesBufferKafka.Len() < 512 {
		return
	}
...
func (k *TenaciousProducerWithBuffer) flush() {
	...
	for {
		...
		domain_error.ProcessSevereError(false, 0, nil, err, "TenaciousProducerWithBuffer :: fail to flush message")
		if err == nil {
			k.bytesBuffer.Reset()
			return
		} else {
...
func (k *TenaciousProducerWithBuffer) flushKafka() {
	if k.bytesBufferKafka.Len() == 0 {
		return
	}
	for {
		kafkaMessage := &sarama.ProducerMessage{
			...
			Value: sarama.ByteEncoder(k.bytesBufferKafka.Bytes()),
		}
		_, _, err := k.producer.SendMessage(kafkaMessage)
		domain_error.ProcessSevereError(false, 0, nil, err, "TenaciousProducerWithBuffer :: fail to send kafka message")
		if err == nil {
			k.bytesBufferKafka.Reset()
			return
		} else {
			time.Sleep(time.Second)
		}
