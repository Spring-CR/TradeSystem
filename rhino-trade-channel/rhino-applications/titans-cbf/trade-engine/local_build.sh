...
PLUG_VERSION=1.0.2.8   # 插件版本
IS_PROD=1              # 1 表示 true，即生产环境；0 表示 false，即测试环境

COMPONENT=trade-engine-titans-cbf
...
#echo "CMD [\"/bin/sh\", \"-c\", \"/trade-engine-titans-cbf 2>&1 | tee -a /opt/log/trade-engine.log\"]" >> ./docker/Dockerfile
...
RUN echo "/opt/log/trade-engine.log {"  > /etc/logrotate.d/trade-engine && \\
    ...
    echo "  rotate 7"                  >> /etc/logrotate.d/trade-engine && \\
