...
func DomainOrderToRawOrderFix44(order *schema.TradeOrder) (rawOrder newordersingle44.NewOrderSingle) {
	...
	// 构建fix4.2订单数据结构
	rawOrder = newordersingle44.New(clOrdId, side, transactTime, ordType)

	rawOrder.SetField(tag.TransactTime, transactTime)
	rawOrder.SetField(tag.Symbol, symbol)
	if order.HandlInst != "" {
		handlInst := field.NewHandlInst(enum.HandlInst(order.HandlInst))
		rawOrder.SetField(tag.HandlInst, handlInst)
	}
	...
	if order.IDSource != "" {
		...
		rawOrder.SetField(tag.SecurityID, field.NewSecurityID(order.IDSource))
	}
	...
	if order.OpenClose != "" {
		...
		rawOrder.SetPositionEffect(enum.PositionEffect(order.OpenClose))
	}
