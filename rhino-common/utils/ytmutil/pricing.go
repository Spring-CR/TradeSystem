...
// 计算YTM和DirtyPrice
func (b *BondPricingEngine) CalculateYTMAndDirtyPrice(ytm_n_reserve int, dirtyPrice_n_reserve int, needYtm bool) (float64, float64, error) {
...
func (u *YtmUtil) ComputeDirtyPrice(applicationCfg *domain_cfg.ApplicationCfg, settlType string, tradeOrder *schema.TradeOrder, parValue int, cleanPrice float64, symbolData map[string]interface{}, needYtm bool) (dirtyPrice float64, ytm float64, err error) {
	...
	if settlType == "T+0" {
		...
		settleDate = u.t1DayCache[currDate]
		if settleDate == "" {
	...
	log.Printf("ComputeDirtyPrice, settleDate:%v, couponType:%v, startDate:%v, maturityDate:%v, dayCount:%v, exchMarket:%v, bondType:%v, parValue:%v, issuePrice:%v, couponRate:%v, cashTimes:%v, cleanPrice:%v\n", settleDate, couponType, startDate, maturityDate, dayCount, exchMarket, bondType, parValue, issuePrice, couponRate, cashTimes, cleanPrice)
	...
	ytm_n_reserve := 4
	dirtyPrice_n_reserve := 8
	ytm, dirtyPrice, err1 = engine.CalculateYTMAndDirtyPrice(ytm_n_reserve, dirtyPrice_n_reserve, needYtm)
	if err1 != nil {
